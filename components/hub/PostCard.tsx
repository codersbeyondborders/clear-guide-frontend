...
interface PostCardProps {
  ...
  onUpdate?: (updated: HubPost) => void
  /** When true, body is not truncated and comments load inline */
  expanded?: boolean
}

export function PostCard({ post: initialPost, viewerId, isAuthenticated, onDelete, onUpdate, expanded = false }: PostCardProps) {
  const [post, setPost] = useState<HubPost>(initialPost)
  ...
  function handlePostUpdated(updated: HubPost) {
    setPost(updated)
    onUpdate?.(updated)
  }
              ...
              {menuOpen && (
                <div className="absolute right-0 top-8 z-20 rounded-xl border shadow-lg py-1 min-w-[140px]"
                  ...
                  <button type="button" onClick={handleDelete} disabled={deleting}
                    className="flex items-center gap-2 w-full px-3 py-2 text-sm transition-colors hover:bg-background-subtle focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring border-t"
